...
locals {
  state_dynamodb_table_name = "${var.aws_dynamodb_table}-${random_integer.rand.result}"
  state_bucket_name         = "${var.aws_bucket_prefix}-${random_integer.rand.result}"
}
...
resource "aws_iam_group" "bucket_full_access" {
  name = "${local.state_bucket_name}-full-access"
}

resource "aws_iam_group" "bucket_read_only" {
  name = "${local.state_bucket_name}-read-only"
}
